...
export const Route = createFileRoute("/_authenticated/restaurant")({
  head: () => ({ meta: [{ title: "Restaurant — QweekPOS" }] }),
...
function RestaurantLayout() {
  ...
  const tabs = [
    { to: "/restaurant", label: "Overview" },
    { to: "/restaurant/tables", label: "Tables" },
    { to: "/restaurant/orders", label: "Orders (KOT)" },
  ];
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <h1 className="font-display text-2xl font-semibold">Restaurant</h1>
      </div>
      <div className="flex gap-2 border-b border-border/60 overflow-x-auto">
        {tabs.map((t) => {
          const active = t.to === "/restaurant" ? pathname === "/restaurant" : pathname.startsWith(t.to);
          return (
